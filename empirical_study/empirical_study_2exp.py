...
def split_parameters_exp(parameters, setting):
    n_betas = len(setting)
    lambdas = (
        parameters[-2 * n_betas :].reshape(-1, 2).T
    )  # line 0 for lam_1,0 and lam_2,0
...
def fit_betas_exp(parameters, X_train, y_train, setting):
    ...
    train_features, _ = linear_of_kernels_exp(
        returns=X_train,
        setting=setting,
...
def linear_of_kernels_exp(returns, setting, parameters, return_features=False):
    splitted_parameters = split_parameters_exp(
        parameters=parameters, setting=setting
    )
    features = compute_features_exp(
        returns=returns,
        setting=setting,
...
def compute_features_exp(returns, setting, splitted_parameters):
    ...
    for j, key in enumerate(setting):
        i, p = key
        features[i] = {
            k: compute_kernel_weighted_sum(
                x=returns,
                params=[lambdas[0, j], lambdas[1, j], thetas[j]],
                func_power_law=convex_combi_exp_kernel,
                transform=power_to(i),
                result_transform=power_to(k),
            )
            for k in p
        }
    return features
...
def find_optimal_parameters_exp(
    vol,
    index,
    p=1,
    setting=((1, 1), (2, 1 / 2)),
    ...
    Computes the optimal parameters to linearly estimate vol^p using the
        previous returns of index using the convex combination of exponentials
    ...
    :param non_negative_beta: bool. Only valid for the parabolic model. Ensures
        that the beta in front for R_1^2 is positive.
    ...
    # If it's just one value, transform i numpy as np
    # import pandas as pdt to tuple
    ...
    def residuals(parameters):  # , return_diff=False):
        res = -y_train + linear_of_kernels_exp(
    ...
    def jacobian(parameters):
        train_features, pred_train = linear_of_kernels_exp(
            returns=X_train,
            setting=setting,
            parameters=parameters,
            return_features=True,
        ...
        for iter, (i, p) in enumerate(setting):
            ...
            dR_i_dtheta = compute_kernel_weighted_sum(
                x=X_train,
                params=params_i,
                func_power_law=deriv_theta_exp_kernel,
                transform=power_to(i),
                result_transform=identity,
            )

            dR_i_dlam0 = compute_kernel_weighted_sum(
                x=X_train,
                params=params_i,
                func_power_law=deriv_lam0_exp_kernel,
                transform=power_to(i),
                result_transform=identity,
            )

            dR_i_dlam1 = compute_kernel_weighted_sum(
                x=X_train,
                params=params_i,
                func_power_law=deriv_lam1_exp_kernel,
                transform=power_to(i),
                result_transform=identity,
            )
    ...
    if not fixed_initial:
        initial_parameters = optimal_parameters_from_exponentials_exp(
            X=X_train, y=y_train, p=p, setting=setting, plot=False
        )
    ...
    if non_negative_beta:
        lower_bound[
            2
        ] = 0  # Keep the beta of R_1^2 non negative. Only valid for
        # parabole_R_linear_sigma
    ...
    initial_pred_train = linear_of_kernels_exp(
        returns=X_train,
        setting=setting,
        parameters=initial_parameters,
        return_features=False,
    )
    initial_pred_test = linear_of_kernels_exp(
        returns=X_test,
        setting=setting,
        parameters=initial_parameters,
        return_features=False,
    ...
    initial_vol_pred_train = inv_target_transform(
        initial_pred_train * norm_coeff
    )
    ...
    # constraints = {'type': 'ineq', 'fun': constraint_lam_theta, 'args':
    #   (min_lam_array, ), 'jac': jac_constrains}
    #                # 'jac': jac_constrains}
    # bounds = [(lower_bound[i], upper_bound[i]) for i in
    #   range(len(lower_bound))]
    # sol = minimize(residuals, initial_parameters, method='SLSQP',
    #   bounds=bounds, constraints=constraints, jac=jacob)
    opt_params = sol["x"]
    split_opt_params = split_parameters_exp(
        parameters=opt_params, setting=setting
    )
    ...
    test_features, pred_test = linear_of_kernels_exp(
        returns=X_test,
        setting=setting,
        parameters=opt_params,
        return_features=True,
    ...
    split_opt_params[1] = (
        split_opt_params[1] * norm_coeff
    )  # Add correction term to betas
    train_features = OrderedDict(
        [(key, pd.DataFrame(train_features[key])) for key in train_features]
    )
    ...
    for key in train_features:
        ...
        importance[iter : iter + size] = (
            split_opt_params[1][iter : iter + size]
            * train_features[key].std().values
        )
        iter += size

    # keys = ['beta_0', 'betas', 'thetas', 'lambdas']
    split_init_params = split_parameters_exp(
        parameters=initial_parameters, setting=setting
    )
    ...
    ans = {
        "sol": sol,
        "opt_params": params_dict,
        "setting": setting,
        "p": p,
        "norm_coef": norm_coeff,
        ...
        # {keys[i]: split_parameters_exp(parameters=initial_parameters,
        #   setting=setting)[i] for i in
        #                    range(len(keys))},
...
if __name__ == "__main__":
    ...
    max_delta = 1000  # Number of past returns used in the computation of
    # R_{n,t} in business days
