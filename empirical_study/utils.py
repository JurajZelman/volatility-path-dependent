...
def exp_power_law(t, alpha, k):
    ...
    return np.true_divide(
        1 - np.exp(-((k * t) ** alpha)) + eps, ((k * t) ** alpha + eps)
    )
...
def normalized_TSPL(t, alpha, delta):
    return shifted_power_law(t, alpha, delta) / (
...
def find_best_exp(
    ...
    plot=False,
    nlam=2,
    func_power_law=normalized_TSPL,
    lower_lam=None,
):
    """
    Finds the best exponential(if nlam=1) or convex combination of
        exponentials(nlam=2) that fits the func_power_law
    :param pl_params:
    :param fit_period:
    :param lam:
    :param plot:
    ...
    if lam is None:
        opt_params, _ = curve_fit(
            f, TT, shifted_pl, bounds=(lower, upper), maxfev=4000
        )
    else:
        exp_l = f(TT, lam, 1)
    ...
    if len(opt_params) == 2:
        ...
        ans = {
            "lam0": max(opt_params[:2]),
            "lam1": min(opt_params[:2]),
            "c": opt_params[3],
            "theta": opt_params[2]
            if opt_params[0] > opt_params[1]
            else 1 - opt_params[2],
        }
    if plot:
