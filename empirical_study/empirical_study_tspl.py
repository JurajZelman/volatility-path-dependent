...
def optimal_parameters_GJR_parent(
    vol,
    index,
    ...
    def prediction(parameters, returns, return_features=True, parent=parent):
        ...
        R_1 = compute_kernel_weighted_sum(
            x=returns,
            params=parameters[3:],
            func_power_law=shifted_power_law,
            transform=power_to(1),
            result_transform=power_to(1),
        )
        if parent:
            u = parameters[2] - R_1
            # ans = parameters[0] ** 2 + (u ** 2 + 2 * parameters[0] * u) *
            #   (R_1 <= parameters[2])
    ...
    def jacobian(parameters):
        features, pred_train = prediction(
            parameters=parameters, returns=X_train, return_features=True
        )
        R_1 = features[1][1]
        # parameters = [beta_0, beta_1, c, alpha, delta]
        ...
        if parent:
            ...
            # jacob[1] = (2 * parameters[1] * u ** 2 + 2 * parameters[0] * u)
            #   * cond
            # jacob[2] = (2 * parameters[1] ** 2 * u + 2 * parameters[0] *
            #   parameters[1]) * cond
            jacob[0] = 1
            jacob[1] = u * cond
    ...
    if not fixed_initial:
        params = optimal_parameters_from_exponentials_tspl(
            X=X_train,
            y=y_train,
            p=1,
            setting=[(1, (1,))],
        ...
        features, _ = prediction(
            parameters=initial_parameters, returns=X_train, return_features=True
        )
    ...
    upper_bound[
        3
    ] = 100  # to avoid an error when alphas are too large(all kernels are 0)
    ...
    initial_pred_train = prediction(
        parameters=initial_parameters, returns=X_train, return_features=False
    )
    initial_pred_test = prediction(
        parameters=initial_parameters, returns=X_test, return_features=False
    )
    ...
    opt_params = sol["x"]
    train_features, pred_train = prediction(
        parameters=opt_params, returns=X_train, return_features=True
    )
    test_features, pred_test = prediction(
        parameters=opt_params, returns=X_test, return_features=True
    )
    ...
    train_features = OrderedDict(
        [
            (key, pd.DataFrame(train_features[key]) / norm_const)
            for key in train_features
        ]
    )
    test_features = OrderedDict(
        [
            (key, pd.DataFrame(test_features[key]) / norm_const)
    ...
    ans = {
        ...
        "features": features,
        "importance": split_opt_params,
        "initial_parameters": {
            keys[i]: initial_parameters[i] for i in range(5)
        },
...
def fit_arch_model(
    ...
    :param max_delta: int, default 1000. Number of days used to compute the past
        returns for each day
    :param step_delta: if values_of_p is None, try all ARCH(p) for p in
        0:max_delta:step_delta
    :param train_start_date:
    :param test_start_date:
    :param model_class: class of regressor to use. Default is sklearn
        LinearRegression. For the paper, we used LassoCV
    :param test_end_date:
    ...
    test_rmse = mean_squared_error(
        y_true=vol_test, y_pred=test_pred, squared=False
    )
    ...
    train_rmse = mean_squared_error(
        y_true=vol_train, y_pred=train_pred, squared=False
    )
...
if __name__ == "__main__":
    ...
    sol = find_optimal_parameters_tspl(
        vol=vix,
        index=spx,
        p=p,
        setting=setting,
